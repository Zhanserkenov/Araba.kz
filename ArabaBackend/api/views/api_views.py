...
from api.serializers import CategorySerializer1, CategorySerializer2, CarSerializer1, CarSerializer2, CarSerializer3
...
def car_list(request):
    if request.method == 'GET':
        ...
        # serializer = CarSerializer1(cars, many=True)
        # return JsonResponse(serializer.data, safe=False)

    if request.method == 'POST':
        data = json.loads(request.body)
        serializer = CarSerializer1(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data)
        return JsonResponse(serializer.errors, status=400)
...
def car_detail(request, car_id):
    ...
    if request.method == 'GET':
        serializer = CarSerializer2(car)
        return JsonResponse(serializer.data)

    elif request.method == 'PUT':
        data = json.loads(request.body)
        serializer = CarSerializer3(instance=car, data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data)
        return JsonResponse(serializer.errors, status=400)
